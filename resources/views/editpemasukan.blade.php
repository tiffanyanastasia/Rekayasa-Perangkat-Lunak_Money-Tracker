...
<body>
    <div class="container mt-5">
        ...
        <form action="/pemasukan/{{ $pemasukan->id }}/edit" method="post">
            @csrf
            <input type="hidden" name="user" value="{{ auth()->user()->id }}">
            <input type="hidden" id="jumlahawal" name="jumlahawal" value="{{ $pemasukan->jumlah }}">

            <div class="mb-3">
                <label for="jumlah" class="form-label">Jumlah</label>
                <input type="text" class="form-control" id="jumlah" name="jumlah" placeholder="Jumlah" value="{{ $pemasukan->jumlah }}">
            </div>
            ...
            <div class="mb-3">
                ...
                @foreach ($kategori as $k)
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="Kategori_id" id="kategori{{ $k->id }}" value="{{ $k->id }}" {{ $k->id == $pemasukan->kategori_id ? 'checked' : '' }}>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="rekening" name="rekening">
                    @foreach ($rekening as $r)
                        <option value="{{ $r->id }}" {{ $r->id == $pemasukan->rekening_id ? 'selected' : '' }}>{{ $r->nama_rekening }}</option>
                    @endforeach
                </select>
            </div>
